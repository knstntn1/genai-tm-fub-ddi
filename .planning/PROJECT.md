# GenAI Teachable Machine Wikimedia Commons Image Search

## What This Is

This is an extension of the existing GenAI Teachable Machine for students. In the image training workflow, students can already add class examples from the device camera or local files; this project adds a third, very simple option: search Wikimedia Commons for an image and add a selected result directly as training data for the current class.
...
Students can add useful image examples to a class through a trivial Wikimedia Commons search flow without leaving the training-data workflow.
...
- [ ] Add a Wikimedia Commons image-search entry point to the image class training workflow alongside the existing camera and file options.
- [ ] Let students enter a simple search term and browse image-only Wikimedia Commons results for the current class.
...
- [ ] On click, import the selected Wikimedia Commons image into the current class as a normal trainable image sample.
...
- [ ] Add focused tests for Wikimedia Commons search, result rendering, and adding a selected result to class training data.
...
- A backend proxy for Wikimedia Commons in v1 unless browser/CORS behavior makes direct client usage impossible — this repo is currently a static browser SPA.
...
- Persisting Wikimedia Commons attribution metadata in the UI — attribution may be revisited later if product or legal requirements change.
...
The current image-training path already supports camera capture, file import, drag/drop, sample movement/deletion, and browser-side model training. Wikimedia Commons should therefore be integrated as another image sample source rather than as a separate dataset system or model feature.

The app already uses browser `fetch` for external resources such as the Generation AI dataset catalog in `src/util/datasets.ts` and model sharing APIs in `src/workflow/ImageWorkspace/ShareProtocol.tsx`. Wikimedia Commons will become a new external browser API integration and should be isolated behind a small local API/client module so UI components do not depend directly on response shapes.
...
- **Integration**: Use the Wikimedia Commons API for image search and keep API handling isolated from presentation components.
- **Static SPA**: The current project has no backend. Prefer direct browser integration unless Wikimedia Commons access requires a proxy.
- **Training data compatibility**: Imported Wikimedia Commons images must enter the same class sample pipeline as existing image samples so training, save/load, and sample management continue to work.
...
|----------|-----------|---------|
| Add Wikimedia Commons image search as a third image-sample source | Students need an easier way to add class examples without taking photos or managing local files | — Pending |
...
| Integrate selected results into existing class sample state | Training, sample management, and save/load should treat Wikimedia Commons images like normal image examples | — Pending |
...
| Replace OpenVerse with Wikimedia Commons | Wikimedia Commons offers direct browser CORS access without an API key and keeps the existing image-only classroom workflow intact | Implemented 2026-06-11 |
